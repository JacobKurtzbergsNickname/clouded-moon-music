...
async function startDocker() {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      await runCommand('powershell -Command "Start-Process \'C:\\Program Files\\Docker\\Docker\\Docker Desktop.exe\'"');
...
async function containerExists(containerName) {
  try {
    const result = await runCommand(`docker ps -a --filter "name=${containerName}" --format "{{.Names}}"`);
...
async function isContainerRunning(containerName) {
  try {
    const result = await runCommand(`docker ps --filter "name=${containerName}" --filter "status=running" --format "{{.Names}}"`);
    return result.stdout.trim().includes(containerName);
  } catch (error) {
    return false;
  }
...
async function startContainer(containerName) {
  try {
    await runCommand(`docker-compose -f ${COMPOSE_FILE} start ${SERVICE_NAME}`);
    return true;
  } catch (error) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (await isContainerRunning(containerName)) {
        ...
        await runCommand(`docker exec ${containerName} mongosh --eval "db.adminCommand('ping')" --quiet`);
    ...
    if (attempt < retries) {
      log(`Waiting ${RETRY_DELAY_MS / 1000} seconds before retry...`, 'verbose');
...
async function main() {
  try {
    ...
    if (!await waitForMongo(containerName)) {
      ...
      log('Container may be starting. Check logs with: docker logs ' + containerName, 'info');
      process.exit(1);
    }
